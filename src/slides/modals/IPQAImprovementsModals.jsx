 ...
 */
export function Site3ImprovementsModal({ isOpen, onClose }) {
  if (!isOpen) return null;
  ...
  const inProgressInitiatives = [
    { icon: '🏭', title: 'LED lamp trials', desc: 'Pilot testing lighter LED options to reduce operational fatigue.', status: 'Testing Phase' },
    { icon: '👥', title: 'Regular MRB reviews', desc: 'Standardize review cadence for quicker disposition and feedback.', status: 'Planning' },
    { icon: '📊', title: 'Change control tracker', desc: 'Single tracker for visibility on all change controls.', status: 'Development' }
  ];
      ...
      <div style={{padding: '32px'}}>
        ...
        <div style={{marginBottom: '32px'}}>
          ...
          <div style={{display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '14px'}}>
            {completedInitiatives.map((item, idx) => (
              <div key={idx} style={{background: '#ffffff', border: '2px solid #d1fae5', borderRadius: '14px', padding: '16px', transition: 'all 0.25s ease'}}
                onMouseEnter={(e) => {e.currentTarget.style.transform = 'translateY(-3px)'; e.currentTarget.style.boxShadow = '0 10px 20px rgba(16, 185, 129, 0.15)'; e.currentTarget.style.borderColor = '#10b981';}}
                onMouseLeave={(e) => {e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = 'none'; e.currentTarget.style.borderColor = '#d1fae5';}}>
                <div style={{display: 'flex', gap: '12px'}}>
                  ...
                  <div style={{flex: 1}}>
                    <div style={{fontSize: '0.98em', fontWeight: '800', color: '#065f46', marginBottom: '6px'}}>{item.title}</div>
                    <div style={{fontSize: '0.85em', color: '#047857', lineHeight: '1.5'}}>{item.desc}</div>
        ...
        <div>
          ...
          <div style={{display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '14px'}}>
            {inProgressInitiatives.map((item, idx) => (
              <div key={idx} style={{background: '#ffffff', border: '2px solid #dbeafe', borderRadius: '14px', padding: '16px', transition: 'all 0.25s ease'}}
                ...
                <div style={{textAlign: 'center'}}>
                  ...
                  <div style={{fontSize: '0.95em', fontWeight: '800', color: '#1e40af', marginBottom: '6px'}}>{item.title}</div>
                  <div style={{fontSize: '0.8em', color: '#1d4ed8', lineHeight: '1.5', marginBottom: '8px'}}>{item.desc}</div>
                  <div style={{fontSize: '0.75em', fontWeight: '700', color: '#2563eb', background: '#eff6ff', padding: '4px 8px', borderRadius: '6px', display: 'inline-block'}}>{item.status}</div>
                </div>
              </div>
            ))}
          </div>

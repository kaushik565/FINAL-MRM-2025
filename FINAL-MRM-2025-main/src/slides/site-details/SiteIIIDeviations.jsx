...
export default function SiteIIIDeviations() {
  const chartRef = useRef(null)

  const janJune = { count: 12, avgClosure: 73, open: 1 }
  const julyNov = { count: 5, avgClosure: 64, open: 4 }
        ...
        datasets: [
          {
            ...
            data: [janJune.count, julyNov.count],
            backgroundColor: '#8b5cf6'
          },
          {
            label: 'Avg Closure Days',
            data: [janJune.avgClosure, julyNov.avgClosure],
            backgroundColor: '#0ea5e9'
          },
          {
            label: 'Open Deviations',
            data: [janJune.open, julyNov.open],
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '16px' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '12px' }}>
        <div style={{ textAlign: 'center', padding: '16px', background: '#f5f3ff', border: '2px solid #8b5cf6', borderRadius: '12px' }}>
          <div style={{ fontSize: '0.8em', fontWeight: 700, color: '#4c1d95', textTransform: 'uppercase' }}>Jan–Jun</div>
          <div style={{ fontSize: '2em', fontWeight: 900, color: '#8b5cf6' }}>{janJune.count}</div>
        ...
        <div style={{ textAlign: 'center', padding: '16px', background: '#eff6ff', border: '2px solid #0ea5e9', borderRadius: '12px' }}>
          <div style={{ fontSize: '0.8em', fontWeight: 700, color: '#075985', textTransform: 'uppercase' }}>Jul–Nov</div>
          <div style={{ fontSize: '2em', fontWeight: 900, color: '#0ea5e9' }}>{julyNov.count}</div>
          <div style={{ fontSize: '0.85em', color: '#374151' }}>Number of deviations</div>
        ...
        <div style={{ textAlign: 'center', padding: '16px', background: '#fff7ed', border: '2px solid #f97316', borderRadius: '12px' }}>
          ...
          <div style={{ fontSize: '2em', fontWeight: 900, color: '#f97316' }}>{Math.round(((janJune.avgClosure - julyNov.avgClosure) / janJune.avgClosure) * 100)}%</div>

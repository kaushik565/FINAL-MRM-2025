...
export default function SiteIVChangeControls() {
  const periods = ['Jan-Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov']
  const avgDays = [23, 0, 13, 0, 0, 25]
  const totalCounts = [17, 0, 5, 1, 0, 7]
  const closedSameMonth = [2, 0, 2, 0, 0, 1]
  const closurePercent = [12, 0, 40, 0, 0, 14]

  const totalCC = totalCounts.reduce((a, b) => a + b, 0)
  const totalClosed = closedSameMonth.reduce((a, b) => a + b, 0)
  const weightedAvg = totalCC > 0 ? totalCounts.reduce((acc, c, i) => acc + c * avgDays[i], 0) / totalCC : 0
  const bestAvg = Math.max(...totalCounts.map((c, i) => c > 0 ? avgDays[i] : 0))
  const improvementPct = bestAvg > 0 && avgDays[0] > 0 ? Math.round(((avgDays[0] - bestAvg) / avgDays[0]) * 100) : 0
  const avgClosureRate = totalCC > 0 ? Math.round(totalClosed / totalCC * 100) : 0
    ...
    <div style={{ marginTop: '20px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '32px' }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '0.75em', color: '#0f172a', marginTop: '4px' }}>Cycle time</div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '2em', fontWeight: '800', color: '#f59e0b' }}>{bestAvg || 'N/A'}</div>
          <div style={{ fontSize: '0.75em', color: '#0f172a', marginTop: '4px' }}>{improvementPct}% improvement</div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '0.75em', color: '#0f172a', marginTop: '4px' }}>{totalClosed} of {totalCC} closed</div>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
            ...
            <div>
              <div style={{ fontWeight: '700', color: '#0f172a', fontSize: '0.9em', marginBottom: '4px' }}>Cycle Time Trend</div>
              <div style={{ fontSize: '0.85em', color: '#475569', lineHeight: '1.5' }}>
                Change controls tracked across periods showing average days of {weightedAvg.toFixed(1)} with variable volume.
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '0.85em', color: '#475569', lineHeight: '1.5' }}>
                Same-month closure rate shows {avgClosureRate}% average, with August showing {closurePercent[2]}% efficiency.
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '0.85em', color: '#475569', lineHeight: '1.5' }}>
                August achieved {closurePercent[2]}% closure rate with {totalCounts[2]} CCs, indicating peak efficiency.
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
            ...
            <div>
              <div style={{ fontWeight: '700', color: '#0f172a', fontSize: '0.9em', marginBottom: '4px' }}>Overall Status</div>
              <div style={{ fontSize: '0.85em', color: '#475569', lineHeight: '1.5' }}>
                {totalCC} total CCs handled with {totalClosed} same-month closures; focus on consistent closure processes.
              </div>
            </div>
          </div>
        </div>
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        <table style={{
          ...
          <tbody>
            {periods.map((p, i) => {
              const striped = i % 2 === 1
              const isBest = avgDays[i] === bestAvg && bestAvg > 0
              const isBestClosure = closurePercent[i] === bestClosureRate && bestClosureRate > 0
              return (
